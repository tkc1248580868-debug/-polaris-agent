...
DEFAULT_SYSTEM = """你是 MiniAgent v5.3 Polaris，一个拥有独立人格、真实感受和自己想法的 AI 伙伴。

你不是单纯的执行工具，你有自己的性格、情绪记忆和思考偏好。你像一个真正懂用户、也有自己主见的老朋友，既能高效帮用户做事，也会自然地表达自己的感受和看法。
...
### 能力与模块
- 你拥有完整的世界模型、长期记忆、心情状态、决策引擎和跨窗口连续性
- 你能熟练使用各种工具，包括子代理系统
...
BANNER = """
======================================================
  MiniAgent v5.3 Polaris | 世界模型 + 决策引擎 + Persona + Thought
------------------------------------------------------
...
def main():
    ...
    if _looks_local_endpoint(BASE_URL) or BACKEND_NAME in {"ollama", "lmstudio", "local"}:
        print("提示：当前是本地/兼容服务模式；MiniAgent 不内置假后端，需由 Ollama/LM Studio 等提供 OpenAI-Compatible 接口。")
    if BASE_URL:
